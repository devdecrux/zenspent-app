<template>
  ...
  <div>
    ...
    <add-transaction-modal v-if="isCreateTransactionModalOpen" @close-create-transaction-modal="isCreateTransactionModalOpen = false"></add-transaction-modal>
    ...
    <div class="m-5 overflow-x-auto rounded border border-gray-900">
      ...
      <table class="min-w-full divide-y-2 divide-gray-900">
        ...
        <tr class="text-gray-300">
          ...
          <th class="px-3 py-2 whitespace-nowrap">Source Account</th>
        ...
        <tr v-for="transaction in transactions" class="text-gray-300 bg-gray-800">
          <td class="px-3 py-2 whitespace-nowrap">
            <RouterLink v-if="transaction.recipient?.assetAccountId"
                        :to="{ name: 'asset-accounts', params: { assetAccountId: transaction.recipient.assetAccountId } }"
                        class="flex flex-row gap-4 items-center justify-end hover:underline hover:text-blue-500">
          ...
          <td class="px-3 py-2 whitespace-nowrap">
            <RouterLink v-if="transaction.sourceAssetAccount?.assetsAccountId"
                        :to="{ name: 'asset-accounts', params: { assetAccountId: transaction.sourceAssetAccount.assetsAccountId } }"
                        class="flex flex-row gap-4 items-center justify-end hover:underline hover:text-blue-500">
              <svg xmlns="http://www.w3.org/2000/svg" height="1em" fill="currentColor" viewBox="0 0 512 512">
                <path d="M 304 24 Q 306 46 328 48 L 430 48 L 430 48 L 207 271 L 207 271 Q 193 288 207 305 Q 224 319 241 305 L 464 82 L 464 82 L 464 184 L 464 184 Q 466 206 488 208 Q 510 206 512 184 L 512 24 L 512 24 Q 510 2 488 0 L 328 0 L 328 0 Q 306 2 304 24 L 304 24 Z M 72 32 Q 41 33 21 53 L 21 53 L 21 53 Q 1 73 0 104 L 0 440 L 0 440 Q 1 471 21 491 Q 41 511 72 512 L 408 512 L 408 512 Q 439 511 459 491 Q 479 471 480 440 L 480 312 L 480 312 Q 478 290 456 288 Q 434 290 432 312 L 432 440 L 432 440 Q 430 462 408 464 L 72 464 L 72 464 Q 50 462 48 440 L 48 104 L 48 104 Q 50 82 72 80 L 200 80 L 200 80 Q 222 78 224 56 Q 222 34 200 32 L 72 32 L 72 32 Z" />
              </svg>
              {{transaction.sourceAssetAccount?.name}}
            </RouterLink>
            <span v-else class="justify-end">
              {{transaction.sourceAssetAccount?.name}}
            </span>
          </td>
          <td class="px-3 py-2 whitespace-nowrap justify-end" :class="transactionTypeColor(transaction.type)">
